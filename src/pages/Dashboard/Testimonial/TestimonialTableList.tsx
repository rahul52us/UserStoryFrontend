...
useEffect(() => {
	...
	return (
        <Box width="100%" overflowX="scroll" boxShadow={"2xl"} borderColor={"red.200"}>
          <Table variant="striped">
            <Thead bg={"whiteAlpha.900"} stroke={"whiteAlpha.500"} position={'sticky'} top={0} left={0} right={0}>
              <Tr>
                <Th>Image</Th>
                <Th>Name</Th>
                <Th>Description</Th>
            ...
            <Tbody>
              {testimonials.data.map((testimonial: any) => {
                return (
                  <Tr key={testimonial.id} bg={"gray.500"} stroke={"gray.300"} className="table-row">
                    <Td>
                      <Avatar src={testimonial.image} size={"sm"} />
                    </Td>
                    <Td>{testimonial.name}</Td>
                    <Td>
                      <Tooltip label={testimonial.description} hasArrow textAlign={"center"}>
